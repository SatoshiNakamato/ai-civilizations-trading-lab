...
class AutonomousWorld:
    ...
    def __init__(self,runtime,root='world_state',seed=42):
        self.runtime=runtime; self.life=runtime.life; self.world=runtime.world; self.rng=Random(seed); self.root=Path(root); self.root.mkdir(parents=True,exist_ok=True); self.decisions=[]; self.platform=CivilizationPlatform(root=root,seed=seed,active_budget=8); self.endurance=EnduranceController()
    ...
    def step(self):
        ...
        self.platform.generation(tick); self.platform.save(); self.decisions=self.decisions[-100:]; collect(); guard=self.endurance.check(tick,self.platform.active_budget); self.platform.active_budget=guard['active_budget']
        snapshot=self.life.snapshot(ids); snapshot.update({'tick':tick,'recent_decisions':[asdict(x) for x in self.decisions[-20:]],'internet_learning':web_learning,'internet':self.world.snapshot(),'civilization':self.platform.snapshot(),'observatory':self.platform.observatory(),'memory':memory_snapshot(),'endurance':guard}); (self.root/'latest.json').write_text(json.dumps(snapshot,separators=(',',':'),default=str),encoding='utf-8'); return snapshot
