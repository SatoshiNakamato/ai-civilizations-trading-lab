...
class EnduranceController:
    ...
    def rss_mb(self) -> float:
        ...
        # Linux/Android: KiB; macOS: bytes.
        return value / 1024.0 if value < 1024 * 1024 * 1024 else value / (1024.0 * 1024.0)
    ...
    def check(self, tick: int, budget: int) -> dict:
        ...
        if self.last_mb >= self.hard_limit_mb:
            level = 'critical'; self.pressure_events += 1; gc.collect(); new_budget = self.minimum_budget
        elif self.last_mb >= self.soft_limit_mb:
            level = 'elevated'; self.pressure_events += 1; new_budget = max(self.minimum_budget, budget // 2); gc.collect()
