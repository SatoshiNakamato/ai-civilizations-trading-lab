...
class Civilization:
    ...
    def step(self,active_ids=None):
        ...
        self.emergence.advance(self.agents.values(),self.tick); self.generation+=1; self.bureau.generation=self.generation; self.evolution.evolve(self.agents.values()); self.events.append(f'tick={self.tick}: active={len(active)}; emergence cycle complete; resolved={resolved}; organizations={len(self.emergence.organizations)}; memes={len(self.emergence.memes)}'); self.events=self.events[-100:]; return self.snapshot()
