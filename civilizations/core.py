...
class Agent:
    ...
    def observe_and_propose(self, tick: int, rng: Random) -> Idea:
        ...
        return Idea(f"{self.archetype}-idea-{tick}-{rng.randrange(1_000_000)}", themes[self.archetype], self.agent_id, generation=tick)
...
class Civilization:
    def __init__(self, size: int = 100, seed: int = 42):
        ...
        self.network = CommunicationNetwork(); self.society = Society(); self._create_population(size)
    ...
    def step(self) -> dict:
        ...
        for agent in self.agents.values():
            idea = agent.observe_and_propose(self.tick, self.rng); agent.evaluate(idea, self.rng); agent.ideas.append(idea); self.global_ideas.append(idea)
        ...
        for idea in champions:
            for peer in self._sample_peers(idea.origin, 3):
                ...
                self.society.talk(self.generation, sender.agent_id, peer.agent_id, idea.title, message, useful)
                ...
                if idea.fitness >= 0.55 and self.rng.random() < peer.curiosity:
                    child = mutate(idea, peer, self.rng); peer.evaluate(child, self.rng); peer.ideas.append(child); self.global_ideas.append(child)
                    self.society.record_knowledge(f"{peer.archetype}:{child.title}", child.thesis, peer.agent_id, child.fitness, self.generation)
        if len(champions) >= 2:
            for peer in self._sample_peers("__council__", 5):
                child = crossover(champions[0], champions[1], peer, self.rng); peer.evaluate(child, self.rng); peer.ideas.append(child); self.global_ideas.append(child)
                self.society.record_knowledge(f"{peer.archetype}:{child.title}", child.thesis, peer.agent_id, child.fitness, self.generation)
        self.generation += 1
        self.events.append(f"tick={self.tick}: {len(proposals)} hypotheses; {len(champions)} debated; mutation/crossover produced new candidates"); self.events = self.events[-100:]
    ...
    def snapshot(self) -> dict:
        ...
        return {"tick":self.tick,"generation":self.generation,"agents":len(self.agents),"ideas":len(self.global_ideas),"messages":len(self.network.memory.messages),"society":self.society.snapshot(),"best_agents":[{"id":a.agent_id,"archetype":a.archetype,"capability":round(a.intelligence.capability_score,3),"experience":a.intelligence.experience,"discoveries":a.intelligence.discoveries} for a in best_agents],"top_ideas":[{"title":i.title,"origin":i.origin,"fitness":round(i.fitness,4),"generation":i.generation} for i in top],"events":self.events[-20:]}

if __name__ == "__main__":
    civilization = Civilization(100, 42); print("AI CIVILIZATION ONLINE"); print("======================"); print(f"Population: {len(civilization.agents)}")
    for _ in range(10):
        state=civilization.step(); print(f"\nGeneration {state['generation']}"); print(f"Ideas discovered: {state['ideas']}"); print(f"Messages: {state['messages']}"); print(f"Shared knowledge: {state['society']['knowledge_count']}"); print(f"Conversations: {state['society']['conversation_count']}")
