...
from .frontier import FrontierCivilizationEngine
...
from .forecast_contract import bind_forecast, ForecastContractRegistry
from .market_events import MarketEvent
from markets.universe import MarketUniverse
...
class Civilization:
    def __init__(self,size=100,seed=42,civilization_id='CIV-001',arena=None,market_universe=None):
        self.civilization_id=civilization_id; self.rng=Random(seed); self.tick=0; self.generation=0; self.max_ideas=500; self.max_agent_ideas=20; self.agents={}; self.global_ideas=[]; self.events=[]; self.network=CommunicationNetwork(); self.society=Society(); self.emergence=EmergenceEngine(seed); self.frontier=FrontierCivilizationEngine(seed=seed+1); self.research=ResearchDesk(web_collector=PublicWebCollector()); self.research_bridge=ResearchBridge(self.research,self.research.web_collector); self.bureau=ResearchBureau(self.research.web_collector); self.evolution=CivilizationEvolution(); self.verifier=AlphaVerifier(); self.arena=arena or CivilizationArena(); self.market_universe=market_universe or MarketUniverse(('BTCUSDT','ETHUSDT','SOLUSDT')); self.forecast_contracts=ForecastContractRegistry(); self._validation_cache={}; self._seed_research(); self._create_population(size); self.emergence.seed_population(self.agents.values()); [self.evolution.specialize(a.agent_id,self._evolution_role(a.archetype)) for a in self.agents.values()]
    ...
    def step(self,active_ids=None):
        ...
        self.emergence.advance(self.agents.values(),self.tick); self.frontier.tick_once(); self.generation+=1; self.bureau.generation=self.generation; self.evolution.evolve(self.agents.values()); self.events.append(f'tick={self.tick}: active={len(active)}; emergence cycle complete; resolved={resolved}; market_pairs={len(self.market_universe)}; arena_forecasts={len(self.arena.commitments)}; arena_resolved={len(self.arena.outcomes)}; organizations={len(self.emergence.organizations)}; memes={len(self.emergence.memes)}; frontier_civilizations={len(self.frontier.civilizations)}; frontier_signals={len(self.frontier.signals)}'); self.events=self.events[-100:]; return self.snapshot()
    ...
    def snapshot(self):
        ...
        return {'civilization_id':self.civilization_id,'tick':self.tick,'generation':self.generation,'agents':len(self.agents),'ideas':len(self.global_ideas),'messages':len(self.network.memory.messages),'market_universe':self.market_universe.snapshot(),'forecast_contracts':self.forecast_contracts.snapshot(),'research':self.research.snapshot(),'bureau':self.bureau.snapshot(),'society':self.society.snapshot(),'emergence':self.emergence.snapshot(),'frontier':self.frontier.snapshot(),'evolution':self.evolution.snapshot(),'arena':self.arena.snapshot(),'best_agents':[{'id':a.agent_id,'archetype':a.archetype,'capability':round(a.intelligence.capability_score,3),'experience':a.intelligence.experience,'discoveries':a.intelligence.discoveries} for a in best],'top_ideas':[{'title':i.title,'origin':i.origin,'fitness':round(i.fitness,4),'real_score':round(i.validation_score,6),'return':round(i.validation_return,6),'drawdown':round(i.validation_drawdown,6),'samples':i.validation_samples,'passed':i.validation_passed} for i in top],'events':self.events[-20:]}
