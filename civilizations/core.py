...
class Civilization:
    ...
    def step(self,active_ids=None):
        ...
        self.emergence.advance(self.agents.values(),self.tick); self.generation+=1; self.bureau.generation=self.generation; self.evolution.evolve(self.agents.values()); self.events.append(f'tick={self.tick}: active={len(active)}; emergence cycle complete; resolved={resolved}; market_pairs={len(self.market_universe)}; arena_forecasts={len(self.arena.commitments)}; arena_resolved={len(self.arena.outcomes)}; organizations={len(self.emergence.organizations)}; memes={len(self.emergence.memes)}'); self.events=self.events[-100:]; return self.snapshot()
    ...
    def snapshot(self):
        ...
        return {'civilization_id':self.civilization_id,'tick':self.tick,'generation':self.generation,'agents':len(self.agents),'ideas':len(self.global_ideas),'messages':len(self.network.memory.messages),'market_universe':self.market_universe.snapshot(),'forecast_contracts':self.forecast_contracts.snapshot(),'research':self.research.snapshot(),'bureau':self.bureau.snapshot(),'society':self.society.snapshot(),'emergence':self.emergence.snapshot(),'evolution':self.evolution.snapshot(),'arena':self.arena.snapshot(),'best_agents':[{'id':a.agent_id,'archetype':a.archetype,'capability':round(a.intelligence.capability_score,3),'experience':a.intelligence.experience,'discoveries':a.intelligence.discoveries} for a in best],'top_ideas':[{'title':i.title,'origin':i.origin,'fitness':round(i.fitness,4),'real_score':round(i.validation_score,6),'return':round(i.validation_return,6),'drawdown':round(i.validation_drawdown,6),'samples':i.validation_samples,'passed':i.validation_passed} for i in top],'events':self.events[-20:]}
