...
class CivilizationEvolution:
    ...
    def save(self):
        # Ensure the runtime state directory exists immediately before both writes.
        # This also makes recovery robust if the data directory is removed while the
        # continuous process is running.
        self.path.parent.mkdir(parents=True, exist_ok=True)
        payload=json.dumps({'generation':self.generation,'predictions':[asdict(x) for x in self.predictions],'knowledge':[asdict(x) for x in self.knowledge],'specializations':self.specializations},indent=2)
    ...
    def score_agents(self):
        ...
        for p in self.predictions:
            ...
            r['observations']+=1; r['confidence_sum']+=p.confidence
        ...
        for r in scores.values():
            r['accuracy']=r['correct']/r['resolved'] if r['resolved'] else 0.0; r['resolution_rate']=r['resolved']/r['observations'] if r['observations'] else 0.0; r['validation_pass_rate']=r['validation_passes']/r['resolved'] if r['resolved'] else 0.0; r['avg_confidence']=r['confidence_sum']/r['observations'] if r['observations'] else 0.0; r['leaderboard_score']=0.7*r['accuracy']+0.2*r['validation_pass_rate']+0.1*r['resolution_rate']
