...
class CivilizationEvolution:
    """Persistent evidence-driven memory, real validation feedback and evolution."""
    ...
    def record_prediction(self,agent_id,asset,hypothesis,prediction,confidence,evidence=0,interval="4h"):
        p=Prediction(agent_id,asset,hypothesis,prediction,confidence,evidence,time(),interval=interval); self.predictions.append(p); self.save(); return p
    def resolve_prediction(self,index,outcome,tolerance=0.01,validation_passed=None):
    ...
    def resolve_real_validation(self, results: dict[tuple[str,str], object], min_samples: int = 200, tolerance: float = 0.02):
        """Resolve prior strategy-score predictions using a fresh real-data verification result.

        This is deliberately a revalidation outcome, not a claim of future-price prediction.
        Results must come from a real verifier and must meet the sample gate.
        """
        resolved=0
    ...
    def score_agents(self):
        ...
        for p in self.predictions:
            r=scores.setdefault(p.agent_id,{'observations':0,'resolved':0,'correct':0,'confidence_sum':0.0,'validation_passes':0,'validation_failures':0})
            r['observations']+=1
            r['confidence_sum']+=p.confidence
            if p.outcome is not None:
                r['resolved']+=1; r['correct']+=int(bool(p.correct)); r['validation_passes']+=int(p.validation_passed is True); r['validation_failures']+=int(p.validation_passed is False)
        for r in scores.values():
            r['accuracy']=r['correct']/r['resolved'] if r['resolved'] else 0.0
            r['resolution_rate']=r['resolved']/r['observations'] if r['observations'] else 0.0
            r['validation_pass_rate']=r['validation_passes']/r['resolved'] if r['resolved'] else 0.0
            r['avg_confidence']=r['confidence_sum']/r['observations'] if r['observations'] else 0.0
            # Score rewards resolved correctness and penalizes unsupported confidence.
            r['leaderboard_score']=0.7*r['accuracy']+0.2*r['validation_pass_rate']+0.1*r['resolution_rate']
    ...
    def leaderboard(self,limit=20):
        return [{'rank':i+1,'agent_id':aid,**stats} for i,(aid,stats) in enumerate(list(self.score_agents().items())[:limit])]
    ...
    def snapshot(self):
        return {'generation':self.generation,'predictions':len(self.predictions),'resolved_predictions':sum(p.outcome is not None for p in self.predictions),'knowledge':len(self.knowledge),'specializations':dict(self.specializations),'agent_scores':self.score_agents(),'leaderboard':self.leaderboard()}
...
def rank_ideas(ideas: Iterable["Idea"]) -> list["Idea"]:
    return sorted(ideas,key=lambda x:(x.validation_passed,x.fitness),reverse=True)
