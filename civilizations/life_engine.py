...
@dataclass
class Memory:
    tick:int; kind:str; text:str; importance:float=.5
@dataclass
class Relationship:
    trust:float=0.; bond:float=0.; conflict:float=0.; interactions:int=0
@dataclass
class LifeState:
    energy:float=1.; curiosity:float=.5; belonging:float=.5; security:float=.5; achievement:float=.5; wellbeing:float=.75

class LifeEngine:
    """Persistent individual-life model with bounded memory and relationships."""
    def __init__(self,seed=42,memory_limit=40,relationship_limit=20,legacy_limit=20):
        self.rng=Random(seed); self.memory_limit=max(10,memory_limit); self.relationship_limit=max(4,relationship_limit); self.legacy_limit=max(4,legacy_limit); self.memories:Dict[str,List[Memory]]={}; self.relationships={}; self.states={}; self.self_models={}; self.legacies={}
    def register(self,agent_id,values=None):
    ...
    def inspect(self,agent_id):
        self.register(agent_id); return {'id':agent_id,'state':vars(self.states[agent_id]).copy(),'self_model':dict(self.self_models[agent_id]),'memories':[vars(m).copy() for m in self.memories[agent_id]],'relationships':{k:vars(v).copy() for k,v in self.relationships[agent_id].items()},'legacy':list(self.legacies[agent_id])}
    def remember(self,agent_id,tick,text,kind='experience',importance=.5):
        self.register(agent_id); self.memories[agent_id].append(Memory(tick,kind,str(text)[:500],max(0,min(1,importance)))); self.memories[agent_id]=sorted(self.memories[agent_id],key=lambda m:(m.importance,m.tick),reverse=True)[:self.memory_limit]
    def interact(self,a,b,tick,outcome):
        ...
        for left,right in ((a,b),(b,a)):
            ...
            self.relationships[left]={k:v for k,v in sorted(self.relationships[left].items(),key=lambda kv:kv[1].interactions,reverse=True)[:self.relationship_limit]}; self.remember(left,tick,f'Interaction with {right}: outcome={outcome:.3f}','relationship',.45)
    def experience(self,agent_id,tick,success,novelty=.5):
        self.register(agent_id); s=self.states[agent_id]; success=max(0,min(1,float(success))); s.energy=max(0,min(1,s.energy+.05*success-.03)); s.curiosity=max(0,min(1,s.curiosity+.04*novelty-.01*success)); s.achievement=max(0,min(1,s.achievement+.04*success)); s.wellbeing=max(0,min(1,.45*s.energy+.35*s.belonging+.20*s.achievement)); self.remember(agent_id,tick,f'Outcome={success:.3f}; novelty={novelty:.3f}','experience',min(1,.4+novelty*.4))
    ...
    def teach(self,parent,child,tick):
        ...
        self.legacies[parent]=(self.legacies[parent]+[child])[-self.legacy_limit:]; return lessons
