...
class TradingCivilizationV1:
    ...
    def cycle(self):
        ...
        for o in candidates:
            ...
            result_dict=asdict(result); bankr_plans.append(result_dict); deployments.append(result_dict); existing.add(ticker.symbol); self._event("bankr",agent,result.status,{"ticker":ticker.symbol,"chain":chain,"token_address":result.token_address,"tx_hash":result.tx_hash})
        ...
        if any(x.get("status")=="deployed" for x in deployments): bankr_status="deployed"; bankr_detail=f"autonomous Bankr token-launch execution ({len(deployments)} launch{'es' if len(deployments)!=1 else ''})"
        ...
        telemetry.stage("bankr",bankr_status,len(deployments),bankr_detail); telemetry.stage("on_chain_observation","pending" if any(x.get("status")=="deployed" for x in deployments) else "idle",len([x for x in deployments if x.get("status")=="deployed"]),"launches queued for observation"); telemetry.stage("pnl","ok",0,"portfolio accounting available"); telemetry.stage("learning","ok",len(self.metrics.stats),"strategy book available")
